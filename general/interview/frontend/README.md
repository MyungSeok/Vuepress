...
* 브라한저와 상관 없이 같은 모양을 나타내도록 CSS 를 초기화 하는것을 말함
...
## 점진적 향상법 vs 우아한 성능 저하법

* 점진적 향상법 (Progressive Enhancement)
  * 노력과 시간으로 기능을 향상시키는 방법
  * 기초부터 차곡차곡 발전해 나가는 방법이다.
  * 웹 페이지를 구성할때
    1. HTML 로 마크업을 구성한다.
    2. CSS 로 스타일을 입힌다.
    3. Javascript 로 사용자 경험(UX) 을 향상시킨다.
* 우아한 성능 저하법 (Graceful Degradation)
  * 최신 기술을 환경기반으로 설계 및 개발을 완료하고 레거시한 한경에서 동작하기 위해 유사한 기능을 만들어 동작하게 하는것
  * 각 환경 사용자들을 위해서 별도의 버전 혹은 우회할 수 있도록 기능을 제공한다.
    1. IE11 기준으로만 개발 완료
    2. IE10 으로 사용자 접속
    3. `IE11 브라우저를 사용해주세요` 라고 하는 메세지를 노출하여 IE11 로의 접근을 유도
